...
public class TourGuide {
    ...
    public ArrayList<View> mHighlightedViewList;
    public ArrayList<Shape> mShapeList;
    public int positionViewShowTip = 0;
     ...
     * @return return TourGuide instance for chaining purpose
     */
    public TourGuide playOn(View targetView, Shape shape) {
        if (mHighlightedViewList == null) {
            mHighlightedViewList = new ArrayList<>();
        }
        mHighlightedViewList.add(targetView);
        if (mShapeList == null) {
    ...
    public TourGuide setPositionViewShowTip(int pos) {
        if (pos >= mHighlightedViewList.size()) {
            throw new IndexOutOfBoundsException();
        }
        positionViewShowTip = pos;
        return this;
    }
    ...
    private int getXBasedOnGravity(View highlightedView, int width) {
        int[] pos = new int[2];
    ...
    //TODO: move into Pointer
    private int getYBasedOnGravity(View highlightedView, int height) {
        int[] pos = new int[2];
    ...
    protected void setupView() {
        ...
        final int size = mHighlightedViewList.size();
        boolean startView = true;
        for (final View mHighlightedView : mHighlightedViewList) {
    ...
    private void startView() {
        ...
        mFrameLayout = new FrameLayoutWithHole(mActivity, mHighlightedViewList, mMotionType, mOverlay, mShapeList);
    ...
    private void handleDisableClicking(FrameLayoutWithHole frameLayoutWithHole) {
        ...
        // 2. if overlay listener is not provided, check if it's disabled
            ...
            frameLayoutWithHole.enforceMotionType();
    ...
    private void setupToolTip() {
        ...
        if (mToolTip != null) {
            ...
            if (mToolTip.getCustomView() == null) {
                ...
                mToolTipViewGroup = layoutInflater.inflate(R.layout.tooltip, parent, false);
                ...
                if (mToolTipViewGroup.getLayoutParams() == null)
                    mToolTipViewGroup.setLayoutParams(new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
                layoutParams = (FrameLayout.LayoutParams) mToolTipViewGroup.getLayoutParams();
            }
            ...
            int[] pos = new int[2];
            if (!mHighlightedViewList.isEmpty()) {
                View highlightedView = mHighlightedViewList.get(positionViewShowTip);
                highlightedView.getLocationOnScreen(pos);
            }
            ...
            if (mToolTip.mGravityType == ToolTip.GRAVITY_TYPE.FOCUS) {
            ...
            mToolTipViewGroup.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    if (mToolTip.mGravityType == ToolTip.GRAVITY_TYPE.FOCUS) {
                        ...
                        layoutParams.setMargins((int) mToolTipViewGroup.getX(), fixedY, layoutParams.rightMargin, layoutParams.bottomMargin);

                        parent.requestLayout();
                    } else if (mToolTip.mGravityType == ToolTip.GRAVITY_TYPE.SCREEN_FOCUS) {
                        int fixedY;
                        int toolTipHeightAfterLayouted = mToolTipViewGroup.getHeight();
                        fixedY = getYForTooTip(mToolTip.mGravity, toolTipHeightAfterLayouted, targetViewY, adjustment);
                        layoutParams.setMargins(0, fixedY, layoutParams.rightMargin, layoutParams.bottomMargin);
                        parent.requestLayout();
                    }
                }
            });
            ...
            layoutParams.setMargins(resultPoint.x, resultPoint.y, layoutParams.rightMargin, layoutParams.bottomMargin);
        }
    }
    ...
    private int getYForTooTip(int gravity, int toolTipMeasuredHeight, int targetViewY, float adjustment) {
        ...
        } else if ((gravity & Gravity.CENTER) == Gravity.CENTER) { // this is center
            if (((gravity & Gravity.LEFT) == Gravity.LEFT) || ((gravity & Gravity.RIGHT) == Gravity.RIGHT)) {
    ...
    private FloatingActionButton setupAndAddFABToFrameLayout(final FrameLayoutWithHole frameLayoutWithHole) {
        ...
        invisFab.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                View highlightedView = mHighlightedViewList.get(positionViewShowTip);
                params.setMargins(getXBasedOnGravity(highlightedView, invisFab.getWidth()), getYBasedOnGravity(highlightedView, invisFab.getHeight()), 0, 0);
            }
        });
